...
path = r"E:\Lenovo Ideapad 330\company-material\digital-workforce-transformation\ai-upskill-10\key-vault\langsmith\api.key"
with open(path) as f:
...
def create_validator_node(llm):
    def validator_node(state: AgentState):
        ...
        # # -----------------------------------------------
        if state.get("status") == "TOOLERROR":
            print("VALIDATOR RECEIVED TOOLERROR STATUS! Marking as INVALID without LLM check.")
            return {**state, "status": "TOOLERROR"}   
        # # -----------------------------------------------
        prompt = f"""
Validate result.
...
def route(state: AgentState):
    ...
    if state["status"] == "VALID":
        return END
    # #-------------------------------------------------------
    elif state["status"] == "TOOLERROR":
        print("ROUTER DETECTED TOOLERROR STATUS! Ending execution without retrying.")
        return END
    # #-------------------------------------------------------
    else:
        state["retries"] += 1
...
if __name__ == "__main__":
    ...
    with open(r"E:\Lenovo Ideapad 330\company-material\digital-workforce-transformation\ai-upskill-10\key-vault\groq\api.key") as f:
